...
function handleFatalError(e, logger = new utils_1.DefaultLogger('◈')) {
  ...
  console.log(e)
  if (process.env.JEST == null) (0, node_process_1.exit)(1)
}
...
async function cleanup() {
  try {
    await node_fs_1.promises.stat(tmpMeshLocation).then((r) => {
      if (r.isFile()) return node_fs_1.promises.unlink(tmpMeshLocation)
    })
  } catch (e) {
    // ignore
  }
...
const main = async () => {
  ...
  await node_fs_1.promises.writeFile(
    `${meshDir}/.mesh.ts`,
    `export * from '${relativePath.split(node_path_1.default.sep).join('/')}.mesh'`,
    { encoding: 'utf8' },
  )
